...
% 15/07/2019
% just a random pmf with P^K probabilities
...
%        apt = all_possible_tuples;
		for t_i = 1:length(n_samples)
			...
			for trial=1:n_trials
                ...
                % Here we get a random pmf with cardinality P^K
                joint_pmf  = generate_random_pmf(PK)';
                %plot(joint_pmf)
                %hold off
...
saved_sim_str = strftime("_end_%d_%m_%Y_%H_%M_sim_total_corr_pure_ICA_15_07_2019",localtime(time()));
