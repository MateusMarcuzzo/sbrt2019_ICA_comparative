...
% 05/08/2019
% We simulate for ICA, a pmf of support
% P^K, where the distribution is the binomial, with
% P_succ in the name as x (never 0 or 1)
...
% P_succ on the binomial pmf
% obs: the filename does not tell this value
P_succ = 0.4;
...
%        apt = all_possible_tuples;
		for t_i = 1:length(n_samples)
			...
			for trial=1:n_trials
                ...
                % Here we get the binomial PMF with support on P^K symbols
                % P_succ = 0.8

                joint_pmf  = binopdf(0:(PK-1),PK-1,P_succ);

                %just checking:
				% generate_binomial_pmf(PK,P_succ)'
				% The above version is not optimized, but must match the 
				% statistics package function.

                %plot(joint_pmf)
                %hold off
                
                % here we shuffle
                % rand_indices = randperm(numel(joint_pmf));
                % joint_pmf = joint_pmf(rand_indices);
                %plot(joint_pmf)
                %hold off
...
end_time_str = strftime("_end_%d_%m_%Y_%H_%M",localtime(time()));

P_succ_times_10 = P_succ*10;
saved_sim_str = sprintf("_sim_pure_ICA_binomialp0%d_05_08_2019",P_succ_times_10);
saved_sim = sprintf("%s%s%s",start_time_str,end_time_str,saved_sim_str)
save(saved_sim)
